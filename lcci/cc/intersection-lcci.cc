...
class Solution {  // 高等数学知识，做叉积
   public:
    ...
    vector<double> intersection(vector<int>& start1, vector<int>& end1,
        ...
        int acx = cx - ax, acy = cy - ay, abx = bx - ax, aby = by - ay,
            adx = dx - ax, ady = dy - ay;
        int cax = ax - cx, cay = ay - cy, cbx = bx - cx, cby = by - cy,
            cdx = dx - cx, cdy = dy - cy;
        // 叉积运算
        ...
        // 计算三角形 PQM 的面积
        auto getArea = [&](int px, int py, int qx, int qy, int mx,
                           int my) -> double {
            int mpx = px - mx, mpy = py - my, mqx = qx - mx, mqy = qy - my;
            return fabs(double(0.5) * cross(mpx, mpy, mqx, mqy));
        };
        // 定比分点
        double ck = getArea(ax, ay, bx, by, cx, cy),
               dk = getArea(ax, ay, bx, by, dx, dy);
        double k = ck / dk;
